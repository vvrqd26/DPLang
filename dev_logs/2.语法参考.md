...
# 调用示例
ma20 = MA(close)              # 使用默认 period=20
ma5 = MA(close, 5)            # 指定 period=5
boll_std = BOLL(close, 20, 2) # 全部指定
boll_default = BOLL(close)    # 使用默认 period=20, std_multiplier=2
```
...
# 链式比较 (新特性)
合理估值 = 0 < pe < 20
中等波动 = 0.02 < volatility < 0.05
# 等价于: (0 < pe) and (pe < 20)

# when 表达式 (新特性)
# 模式匹配风格的条件表达式，替代复杂的三元嵌套
grade = when score >= 90 -> "A", score >= 80 -> "B", score >= 60 -> "C", else -> "不及格"

level = when price > 100 and volume > 1000000 -> "强势",
             price > 100 -> "中等",
             price > 80 -> "弱势",
             else -> "观望"

# when 表达式无 else 分支，无匹配时返回 null
